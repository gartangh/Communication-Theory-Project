...
counts = zeros(1, length(samples_bits)/2);
for k = 0:length(samples_bits)/2-1
    bool_error = 1;
    while bool_error
        counts(k+1) = counts(k+1)+1;
        samples_bits_part = Channel_Coding.Encode_inner(samples_bits(k*2+1:(k+1)*2), [1 1 0 0 1 1 0 1 1]);
        ...
        [result_bits_decoded_part, bool_error] = Channel_Coding.Decode_inner(result_bits, [1 1 0 0 1 1 0 1 1]);
        result_bits_decoded(k*2+1:(k+1)*2) = result_bits_decoded_part;
    end
end
