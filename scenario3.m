...
result_bits_decoded = zeros(1, length(samples_bits_coded)/14*10);
counts = zeros(1, length(samples_bits_coded));
for k = 0:length(samples_bits_coded)/14-1
    bool_error = 1;
    while bool_error
        counts(k+1) = counts(k+1)+1;
        samples_bits_part = samples_bits_coded(k*14+1 : (k+1)*14);
        ...
        [result_bits_decoded_part, bool_error] = Channel_Coding.Decode_outer(result_bits);
        result_bits_decoded(k*10+1:(k+1)*10) = result_bits_decoded_part;
    end
end
...
disp(['Gemiddeld aantal bits = ' num2str(sum(counts)*14/length(result))]);
disp(['GKA = ' num2str(GKA)]);

%{
BER = 6.48e-05
Gemiddeld aantal bits = 5.3118
GKA = 35.7451
%}